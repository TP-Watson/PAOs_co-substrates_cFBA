...
def cFBA_backbone_from_S_matrix(S_matrix):
    ...
    imbalanced_indices = list(map(int, input("Enter the indices of imbalanced metabolites (comma-separated): ").split(',')))
    ...
    if use_capacities == "yes":
        ...
        for i, metabolite in enumerate(imbalanced_metabolites, 1):
            print(f"{i}. {metabolite}")
        catalyst_indices = list(map(int, input("Enter the indices of catalyst metabolites (comma-separated): ").split(',')))
...
def generate_cFBA_excel_sheet(S_matrix, data, output_file_name):
    ...
    if data['use_capacities']:
        ...
        b_cap_df = pd.DataFrame(0, index=data['Imbalanced metabolites'], columns=list(range(1, num_catalyzers + 1)))
...
def excel_to_sbml(excel_file, output_file):
    ...
    print(f"SBML document with metabolites information and catalysis annotations has been created and saved to {output_file}.")
...
def extract_imbalanced_metabolites(species, mets, S):
    ...
    return indices_balanced, indices_imbalanced, imbalanced_mets, balanced_mets, w, Sb, Si
...
def extract_kinetic_parameters(reactions):
    ...
    for reaction_id, reaction_data in reactions.items():
        ...
        lb_values = [kinetic_law_data.get(f'LB_{i}', 0) for i in range(len(kinetic_law_data)) if f'LB_{i}' in kinetic_law_data]  # Get LB_i values or default to 0
        # Extract upper bounds for kinetic parameters
        ub_values = [kinetic_law_data.get(f'UB_{i}', 0) for i in range(len(kinetic_law_data)) if f'LB_{i}' in kinetic_law_data]  # Get UB_i values or default to 0
...
def generate_LP_cFBA(sbml_file, quotas, dt):
    ...
    indices_balanced, indices_imbalanced, imbalanced_mets, balanced_mets, w, Sb, Si = extract_imbalanced_metabolites(species, mets, S)
    ...
    vk = np.array([[Variable(f'{rxns[j]}__{i}', lb=low_b_var[j,i-1], ub=upp_b_var[j,i-1]) for j in range(nr)] for i in range(1, nt)]).T
